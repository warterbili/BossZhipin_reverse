...
**不伪造 Boss 直聘的浏览器环境，让真实登录态 Chrome 执行请求。**
...
### `core/patching.py` — 纯 patch 引擎

运行时 mitm 与 `scripts/validate_boss_patches.py` 共用同一实现：先做表达式替换，再倒序做大括号配对的函数体替换。
这保证“离线验证通过”与“mitm 真实会如此改写”使用同一条代码路径。
...
### `sites/boss/` — Boss 能力边界

当前只支持 Boss 直聘。`SitePlugin` 抽象仅用于隔离 Boss 特定能力：
